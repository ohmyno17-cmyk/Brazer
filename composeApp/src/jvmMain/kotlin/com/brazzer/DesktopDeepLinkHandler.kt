 ...
 * - brazzer://open-app?url=<encoded_url>  (redirected from website)
 * - brazzer://watch?v=VIDEO_ID            (direct scheme)
 * - brazzer://playlist?list=PLAYLIST_ID   (direct scheme)
 * - brazzer://channel/CHANNEL_ID          (direct scheme)
 * - brazzer://album?id=ALBUM_ID           (direct scheme)
 * - https://brazzer.org/app/...            (web URL passed via args)
 */
object DesktopDeepLinkHandler {
    private const val TAG = "DesktopDeepLinkHandler"

    private val pendingUriFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "brazzer_pending_deeplink.txt")
    }
     ...
     *
     * Conversion rules:
     * 1. brazzer://open-app?url=<encoded_url>
     *    → Extract the `url` param and use it as intent data
     *
     * 2. brazzer://watch?v=xxx, brazzer://playlist?list=xxx, etc.
     *    → Convert to https://brazzer.org/app/watch?v=xxx format
     *      so App.kt handles it uniformly via the brazzer.org branch
     *
     * 3. https://brazzer.org/app/... or YouTube URLs
     *    → Pass through as-is
     */
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // brazzer://open-app?url=<encoded_url>
            parsed.scheme == "brazzer" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // brazzer://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // brazzer://watch?v=xxx → https://brazzer.org/app/watch?v=xxx
            // brazzer://playlist?list=xxx → https://brazzer.org/app/playlist?list=xxx
            // brazzer://channel/UCxxx → https://brazzer.org/app/channel/UCxxx
            // brazzer://album?id=xxx → https://brazzer.org/app/album?id=xxx
            parsed.scheme == "brazzer" && parsed.host != null -> {
                val host = parsed.host!!
                val query = parsed.query?.let { "?$it" } ?: ""
                val pathSuffix = parsed.pathSegments.joinToString("/").let {
                    if (it.isNotEmpty()) "/$it" else ""
                }
                val convertedUrl = "https://brazzer.org/app/$host$pathSuffix$query"
                Logger.d(TAG, "Converted brazzer:// to: $convertedUrl")
                Uri.parse(convertedUrl)
            }

            // https://brazzer.org/app/... or YouTube URLs → pass through
            else -> parsed
        }
