...
/**
 * Registers the "brazzer://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\brazzer
 *     (Default) = "URL:brazzer Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "brazzer"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering brazzer:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:brazzer Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\brazzer.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/brazzer.exe
        // So we go: java.home → parent (runtime) → parent (app) → brazzer.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "brazzer.exe")
            if (exeFile.exists()) {
