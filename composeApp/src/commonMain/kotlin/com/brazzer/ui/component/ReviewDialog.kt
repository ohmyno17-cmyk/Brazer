...
import brazzer.composeapp.generated.resources.*
...
fun ReviewDialog(
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(onClick = {
                onDoneReview.invoke()
                uriHandler.openUri("https://github.com/maxrave-dev/brazzer")
            }) {
                Text(
        ...
        title = {
            Text(
                stringResource(Res.string.enjoying_brazzer),
        ...
        text = {
            Text(
                buildAnnotatedString {
                    append(stringResource(Res.string.if_you_enjoy_using_brazzer_star_brazzer_on_github_or_leave_a_review_on))
                    withLink(
                        LinkAnnotation.Url(
                            "https://www.producthunt.com/products/brazzer",
                            TextLinkStyles(style = SpanStyle(textDecoration = TextDecoration.Underline, color = seed)),
                        ) {
                            onDoneReview.invoke()
                            onDismissRequest.invoke()
                            uriHandler.openUri("https://www.producthunt.com/products/brazzer")
                        },
                    ) {
                        append(" ProductHunt")
                    }
